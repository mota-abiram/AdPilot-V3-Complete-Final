...
const EXECUTION_STYLE: Record<ExecutionClassification, string> = {
  "AUTO-EXECUTE": "border-emerald-500/30 bg-emerald-500/10 text-emerald-300",
...
function SectionCard({ severity, card }: { severity: SeverityTier; card: RecommendationCardData }) {
  ...
  const primary = card.solutions[0];
      ...
      <CardContent className="p-5 space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className={cn("text-[10px] font-black uppercase tracking-[0.16em]", style.border, style.tone)}>
                {severity}
              </Badge>
              <Badge variant="outline" className="text-[10px] font-black uppercase tracking-[0.16em] border-border/50 text-foreground/70">
                {card.platform}
              </Badge>
              <Badge variant="outline" className="text-[10px] font-black uppercase tracking-[0.16em] border-border/50 text-foreground/70">
          ...
          <div className="min-w-[220px] space-y-2">
            <div className="rounded-xl border border-border/40 bg-background/50 px-3 py-2">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">Root Cause Chain</p>
              <p className="mt-1 text-xs leading-relaxed text-foreground/85">{card.diagnosis.rootCauseChain.join(" -> ")}</p>
            </div>
            <div className="rounded-xl border border-border/40 bg-background/50 px-3 py-2">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">Primary Solution</p>
              <p className="mt-1 text-xs font-semibold text-foreground">{primary.title}</p>
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <Badge variant="outline" className={cn("text-[10px] font-black uppercase tracking-[0.14em]", EXECUTION_STYLE[primary.classification])}>
                  {primary.classification}
                </Badge>
                <span className="text-[11px] text-muted-foreground">{primary.confidence}% confidence</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="rounded-xl border border-border/40 bg-background/40 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">Diagnosis</p>
            <div className="mt-2 space-y-1.5">
              {card.diagnosis.data.map((line, index) => (
                <p key={index} className="text-xs leading-relaxed text-foreground/85">{line}</p>
              ))}
            </div>
          </div>

          <div className="rounded-xl border border-border/40 bg-background/40 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">Expected Outcome</p>
            <p className="mt-2 text-xs leading-relaxed text-foreground/85">{primary.expectedOutcome}</p>
          </div>
        </div>

        <div className="rounded-xl border border-border/40 bg-background/40 p-3">
          <div className="flex items-center justify-between gap-2">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">Layer Analysis</p>
            <Button
              variant="ghost"
              size="sm"
              className="h-7 px-2 text-[10px] font-black uppercase tracking-[0.14em] text-muted-foreground"
          ...
          <div className="mt-2 grid gap-2 lg:grid-cols-4">
            {[
              ...
              <div key={label} className="rounded-lg border border-border/30 bg-background/50 p-2.5">
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-violet-300">{label}</p>
                <p className="mt-1 text-xs font-semibold text-foreground">{layer.action}</p>
                <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">{layer.reasoning}</p>
              </div>
            ))}
          </div>
          ...
          {card.layerAnalysis.conflicts.length > 0 && (
            <div className="mt-3 rounded-lg border border-amber-500/25 bg-amber-500/8 p-3">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-300">Conflicts</p>
              <div className="mt-2 space-y-1.5">
          ...
          {expanded && (
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
              {card.solutions.map((solution, index) => (
                <div key={`${card.id}-${index}`} className="rounded-lg border border-border/30 bg-background/50 p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant="outline" className={cn("text-[10px] font-black uppercase tracking-[0.14em]", EXECUTION_STYLE[solution.classification])}>
                      {solution.classification}
                    </Badge>
                    <span className="text-[11px] text-muted-foreground">{solution.confidence}% confidence</span>
                    <span className="text-[11px] text-muted-foreground">Risk {solution.risk}</span>
                  </div>
                  <p className="mt-2 text-sm font-semibold text-foreground">{solution.title}</p>
                  <p className="mt-2 text-xs leading-relaxed text-foreground/85">{solution.rationale}</p>
                  {solution.steps.length > 0 && (
                    <div className="mt-3 space-y-1.5">
                      {solution.steps.map((step, stepIndex) => (
                        <p key={stepIndex} className="text-[11px] leading-relaxed text-muted-foreground">
                          {stepIndex + 1}. {step}
                        </p>
                      ))}
                    </div>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="t-page-title text-foreground flex items-center gap-2">
            ...
            <Badge variant="outline" className="border-violet-500/30 text-violet-300 bg-violet-500/10 gap-1.5 py-1">
        ...
        <Button
          variant="outline"
          size="sm"
          className="w-fit border-border/50 text-[10px] font-black uppercase tracking-[0.16em]"
          ...
          <CardContent className="grid gap-3 p-4 md:grid-cols-5">
            {Object.entries(data.layer_contributions).map(([key, value]) => (
              <div key={key} className="rounded-lg border border-border/40 bg-background/50 px-3 py-2">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-muted-foreground">{key.replace(/_/g, " ")}</p>
      ...
      {data?.conflicts?.length ? (
        <div className="rounded-xl border border-amber-500/25 bg-amber-500/8 p-4">
          <div className="flex items-center gap-2 text-amber-300">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.18em]">Visible Layer Conflicts</p>
          </div>
